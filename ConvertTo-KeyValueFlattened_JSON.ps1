function ConvertTo-FlatObject {
    ...
    process {
        ...
        foreach ($prop in $InputObject.psobject.Properties) {
            ...
            #Write-Verbose "$(' ' * 4)Name:  $($prop.name)`n$(' ' * 4)Type: $($prop.value.getType())"
            ...
            $result = [PSCustomObject]@{
                ...
                ArrayCount       = if ($prop.value -is [array]) { $prop.value.count }
                ArrayIndex       = if ($PSBoundParameters.ContainsKey('ArrayIndex')) { $ArrayIndex } else { $null }
                ThisParent       = $thisParent
                ThisName         = $prop.Name
                ThisValue        = $prop.value
                ArrayJoin        = if ($prop.value -is [array]) { ($prop.value | ForEach-Object { $PSItem -join ';' }) -join ',' }
                ArrayToJson      = if ($prop.value -is [array]) { $prop.value | ConvertTo-Json -Depth 10 -Compress }
            }
...
$testObject = [PSCustomObject]@{
    Name          = "TestObject"
    TestSubObject = [PSCustomObject]@{
        SubName  = "SubTestObject"
        SubValue = 'SubObject'
    }
    Value         = 'Test'
    Array         = @('1', '2', '3')
...
ConvertTo-FlatObject $testObject